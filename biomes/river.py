...
def save_checkpoint(lives, health, keys_collected, waves, lily_pads, keys):
    """
    Saves the current game state to "checkpoint.txt". The file stores:
      - Lives, health, and keys collected.
      - For each wave: its position, speed, width, and height.
      - For each lily pad: its position, speed, direction, and radius.
      - For each key: the index of its associated lily pad and its 'collected' flag.
    """
...
def new_game(wm):
    """
    Starts a new game for the River biome.
    Sets default state (lives=3, health=100, etc.) and then enters the game loop.
    """
    ...
    while running:
        ...
        if not game_over:
            ...
            game_over_timer += dt
            if pygame.key.get_pressed()[pygame.K_RETURN] or game_over_timer > 3.0:
                running = False
...
def load_game(wm):
    """
    Loads the saved checkpoint and resumes the game for the River biome.
    In this implementation, we load checkpoint data and use it to initialize the game state.
    If no checkpoint is found, we simply start a new game.
    """
    checkpoint = load_checkpoint()
    if checkpoint is None:
        print("No checkpoint found. Starting new game.")
        new_game(wm)
        return
    ...
    key_color = [1.0, 1.0, 0.0]
    from assets.objects.objects import create_square
    # Create keys in the same way as new_game.
    selected_lily_pads = random.sample(lily_pads, 3)
    keys = []
    for lp in selected_lily_pads:
        key_vertices, key_indices = create_square([0, 0, 0], key_size, key_color)
...
if __name__ == "__main__":
    # For testing, we call new_game if this module is run directly.
    # In practice, your main menu will call new_game(wm) or load_game(wm).
    new_game(None)
